...
def load_genesis4_fields(h5):
    ...
    # Get params
    param = {key:h5[key][0] for key in ['gridpoints',
                                        'gridsize',
                                        'refposition',
                                        'wavelength', 
                                        'slicecount',
    ...
    # slice list
    slist = sorted([g for g in h5 if g.startswith('slice') and g not in ['slicecount', 'slicespacing']])
    
    # 3D complex field
    dfl = np.stack([h5[g]['field-real'][:].reshape(nx,nx) + 1j*h5[g]['field-imag'][:].reshape(nx,nx)  for g in slist], axis=-1)
