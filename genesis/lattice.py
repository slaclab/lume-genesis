...
def lattice_dummies(eles):
    ...
    smax = max([e["s"] for e in eles if e["type"] not in ["comment"]])
    # print(smax)
    dummies = []
    for t in tlat:
        eles2 = tlat[t]
...
def genesis_lattice_from_standard_lattice(
    standard_lattice, include_name=False, include_comment=False
):
    """
    ...
    outlines = [
        "? VERSION = " + str(version),
        "? UNITLENGTH = " + str(unitlength) + " # meters",
    ]
    for t in tlist:
        if t in ["comment", "drift"]:  # Skip these
            continue
