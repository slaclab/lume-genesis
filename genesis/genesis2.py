...
# archive, configure, input_parser, load_archive, load_output, plot, run, write_input
...
class Genesis2(CommandWrapper):
    ...
    def __init__(self, *args, group=None, **kwargs):
        ...
        if self.input_file:
            ...
            assert os.path.exists(infile), f'Genesis2 input file does not exist: {infile}'
    ...
    def get_executable(self):
        ...
        """
        if self.use_mpi:
            exe = lume_tools.find_executable(exename=self.command_mpi, envname=self.command_mpi_env)
        else:
            exe = lume_tools.find_executable(exename=self.command, envname=self.command_env)
        return exe    
