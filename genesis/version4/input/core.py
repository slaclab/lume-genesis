...
T_NameList = TypeVar("T_NameList", bound=AnyNameList)
...
class ProfileArray(NameList):
    ...
    def write(self, base_path: AnyPath) -> pathlib.Path:
        ...
        if "/" in self.temporary_filename:
            raise ValueError(
                ...
                f"the HDF group. "
                f"Got: {self.temporary_filename!r}"
            )
    ...
    def to_profile_file(self) -> ProfileFile:
        ...
        return ProfileFile(
            label=self.label,
            xdata=f"{self.temporary_filename}/{self.x_label}",
            ydata=f"{self.temporary_filename}/{self.y_label}",
            isTime=self.isTime,
...
class _InitialParticles(pydantic.BaseModel):
    ...
    def to_import_distribution(self) -> Importdistribution:
        if not self.temporary_filename:
            raise ValueError("temporary_filename was not set")
...
class InitialParticlesFile(_InitialParticles):
    ...
    that follows the OpenPMD BeamPhysics standard.

    Attributes
    ----------
    filename : str or pathlib.Path

    temporary_filename : str or pathlib.Path
        By default, this is a randomly-generated filename that lume-genesis
        manages for you.  If desirable, you may set a fixed filename relative
        to the main input file.  Path delimiters (such as ``/``) are not
        allowed.
    ...
    type: Literal["InitialParticlesFile"] = "InitialParticlesFile"
    filename: AnyPath = ""
    temporary_filename: AnyPath = ""
...
class InitialParticlesData(_InitialParticles):
    ...
    loaded in Python that follows the OpenPMD BeamPhysics standard.
    ...
    type: Literal["InitialParticlesData"] = "InitialParticlesData"
    data: ParticleData
    temporary_filename: AnyPath = ""
...
class MainInput(pydantic.BaseModel):
    ...
    def by_namelist(self) -> Dict[Type[T_NameList], List[T_NameList]]:
        ...
        by_namelist: Dict[Type[T_NameList], List[T_NameList]] = {}
    ...
    def write_files(
        ...
        for cls in (ProfileArray, InitialParticlesData, InitialParticlesFile):
