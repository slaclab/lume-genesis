...
from . import util
from ._lattice import BeamlineElement, AutogeneratedBeamlineElement
from ._main import (
    Importdistribution,
    NameList,
    Reference,
...
from ..types import AnyPath, ArrayType, Float, ValueType, PydanticParticleGroup
...
def check_allows_reference(field: pydantic.Field) -> bool:
    def check_type(typ) -> bool:
        if typ is Reference:
            return True
        if isinstance(typ, UnionType):
            return any(check_type(typ) for typ in typing.get_args(typ))
        return False

    return check_type(field.annotation)


def get_primary_type(field: pydantic.Field) -> type:
    def is_model(typ):
